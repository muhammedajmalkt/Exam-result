      ...
      try {
        readString(target.result, {
          ...
          complete: async (results) => {
            ...
            // const invalidAssignments = parsed.filter(student => {
            //   const validSections = classSectionMap[student.className];
            //   return !validSections || !validSections.includes(student.section);
            // });

            // if (invalidAssignments.length > 0) {
            //   setError(
            //     `${invalidAssignments.length} students have invalid class-section assignments. ` +
            //     `First error in row #${invalidAssignments[0].rowNumber}: ` +
            //     `${invalidAssignments[0].className} doesn't have section ${invalidAssignments[0].section}`
            //   );
            //   toast.error(`${invalidAssignments.length} invalid class-section assignments`);
            //   if (fileInputRef.current) fileInputRef.current.value = null;
            //   return;
            // }
...
const handleDelete = async (id) => {
  ...
  const generateSampleCSV = () => {
    const csvContent = "data:text/csv;charset=utf-8," +
      "name,regNo,className,section,rollNo\n" +
      "John Doe,REG001,Class 1,A,1\n" +
      "Jane Smith,REG002,Class 1,B,2\n" +
      "Bob Johnson,REG003,Class 2,A,1";
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-gray-300 p-6 ">
        ...
        <div className="mb-6">
          ...
          <p className="mt-2 text-xs text-gray-500">
            CSV must include columns: name, regNo, className, section, rollNo. 
            Max file size: 5MB.
          </p>
        </div>
